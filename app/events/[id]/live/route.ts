...
export async function GET(request: NextRequest, context: any) {
  try {
    ...
    const routePoints = await getEventRoutePoints(eventId);
    ...
    if (!hasPositions) {
      return NextResponse.json({
        ...
        total: 0,
        message: "Tabel live_tracking_positions belum tersedia.",
      });
    }
    ...
    if (!eventCol || !userCol || !latCol || !lngCol) {
      return NextResponse.json({
        ...
        route_points: routePoints,
        total: 0,
        message: "Kolom live_tracking_positions belum lengkap.",
      });
    }
    ...
    const orderCol = updatedCol || idCol || latCol;
    ...
    return NextResponse.json({
      ...
      route_points: routePoints,
      total: rows.length,
      online_count: rows.filter((item: any) => item.is_online).length,
    });
  } catch (error: any) {
