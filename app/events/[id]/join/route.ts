...
function pushInsertValue(input: {
  columns: string[];
  ...
  if (!hasColumn(input.columns, input.columnName)) return;

  input.insertColumns.push(input.columnName);
  input.values.push(input.value);
  input.placeholders.push(`$${input.values.length}`);
}
...
export async function POST(request: NextRequest, context: any) {
  try {
    ...
    const addValue = (columnName: string, value: any) => {
      pushInsertValue({
        columns,
        insertColumns,
        values,
