...
export default function AccountEventLiveViewPage() {
  ...
  const stats = useMemo(() => {
    ...
    const offlineCount = Math.max(0, liveMarkers.length - onlineCount);
      ...
      if (liveResponse) {
        ...
        const liveRows = Array.isArray(liveData?.data)
          ...
          : Array.isArray(liveData?.items)
            ? liveData.items
            : [];
        const routeRows = Array.isArray(liveData?.route_points) ? liveData.route_points : [];

        setLiveMarkers(liveRows);
        setRoutePoints(routeRows);
      } else {
        setLiveMarkers([]);
        setRoutePoints([]);
      }
    ...
    <section className="space-y-4">
      <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          ...
          <div className="rounded-2xl bg-slate-50 p-4">
            <p className="text-xs font-black uppercase text-slate-400">Live</p>
            <p className="mt-1 text-2xl font-black text-slate-950">{stats.liveCount}</p>
          </div>

          <div className="rounded-2xl bg-green-50 p-4">
            <p className="text-xs font-black uppercase text-green-700">Online</p>
            <p className="mt-1 text-2xl font-black text-green-700">{stats.onlineCount}</p>
          </div>
          ...
          <>
            ...
            <section className="relative min-h-[620px] overflow-hidden rounded-[2rem] border border-slate-200 bg-[#edf2f3] shadow-sm">
              ...
              <div className="absolute left-6 top-6 z-10 max-w-md rounded-[1.5rem] border border-slate-200 bg-white/95 p-5 shadow-lg backdrop-blur">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <span className="rounded-full bg-purple-50 px-3 py-1 text-xs font-black uppercase text-purple-700">
                    {stats.liveCount} live marker
                  </span>
                  <span className="rounded-full bg-white px-3 py-1 text-xs font-black uppercase text-slate-600">
                    {routePoints.length} route point
                  </span>
                </div>
              </div>
              ...
              {liveMarkers.length === 0 ? (
                <div className="absolute inset-x-6 bottom-6 z-10 rounded-[1.5rem] border border-slate-200 bg-white/95 p-6 text-center shadow-lg backdrop-blur">
                  ...
                  <p className="mt-2 text-sm font-semibold text-slate-500">
                    Marker akan muncul setelah peserta mulai tracking dan data masuk ke live_tracking_positions.
                  </p>
                </div>
              ) : null}
              ...
              <div className="absolute bottom-6 left-6 z-10 rounded-[1.5rem] border border-slate-200 bg-white/95 p-5 shadow-lg backdrop-blur">
                <p className="text-xs font-black uppercase tracking-wide text-slate-400">Data Real</p>
                <p className="mt-2 max-w-md text-sm font-semibold leading-6 text-slate-600">
                  Halaman ini membaca posisi terakhir peserta dari tabel live_tracking_positions.
                </p>
              </div>
            </section>
              ...
              {liveMarkers.length === 0 ? (
                ...
                <div className="mt-4 overflow-x-auto">
                  <table className="w-full min-w-[920px] border-separate border-spacing-y-3 text-left">
                    ...
                    <tbody>
                      {liveMarkers.map((item) => (
                        <tr
                          key={String(item.position_id || `${item.user_id}-${item.event_id}`)}
...
function LiveMarkerBubble({ marker }: { marker: LiveMarker & { projected?: { x: number; y: number } } }) {
  ...
  const initials = String(marker.full_name || "A")
    .split(" ")
    .filter(Boolean)
    .slice(0, 2)
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <div className={`relative flex h-14 w-14 items-center justify-center rounded-full border-4 border-white text-sm font-black text-white shadow-xl ${marker.is_online ? "bg-green-600" : "bg-slate-500"}`}>
          {initials}
        </div>
