...
export default function NewAdminEventPage() {
  ...
  const previewSlug = useMemo(() => form.slug || createSlug(form.title), [form.slug, form.title]);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!form.title.trim()) return setMessage("Nama event wajib diisi.");
    if (!form.location.trim()) return setMessage("Lokasi event wajib diisi.");
    if (!form.startDate) return setMessage("Tanggal mulai wajib diisi.");
        ...
        <div className="mx-auto flex h-20 max-w-[1440px] items-center justify-between px-4 sm:px-6 lg:px-8">
          <Link href="/admin" className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="mt-1 text-[8px] font-black uppercase leading-[1.05] tracking-wide text-purple-700">Create Event</div>
            </div>
          </Link>
          <Link href="/admin" className="flex items-center gap-2 rounded-lg border border-slate-200 px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50">
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_420px]">
          <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-7">
              ...
              <h1 className="mt-2 text-3xl font-black text-slate-950">Tambah Event</h1>
              <p className="mt-2 text-sm leading-6 text-slate-600">Buat event AMOST baru, lengkap dengan flyer dan route GPX.</p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
              <FormField label="Nama Event" icon={CalendarDays}><input type="text" value={form.title} onChange={(e) => updateField("title", e.target.value)} placeholder="Contoh: Gowes Banyumas" className="form-input" /></FormField>
              <FormField label="Slug" icon={FileText}><input type="text" value={form.slug} onChange={(e) => updateField("slug", createSlug(e.target.value))} placeholder="gowes-banyumas" className="form-input" /></FormField>
              <FormField label="Jenis Event" icon={Ticket}><select value={form.eventType} onChange={(e) => updateField("eventType", e.target.value)} className="form-input"><option value="Sepeda">Sepeda</option><option value="Lari">Lari</option><option value="Trail Run">Trail Run</option><option value="Jalan Sehat">Jalan Sehat</option><option value="Outdoor">Outdoor</option></select></FormField>
              <FormField label="Lokasi" icon={MapPin}><input type="text" value={form.location} onChange={(e) => updateField("location", e.target.value)} placeholder="Banyumas, Jawa Tengah" className="form-input" /></FormField>
              <FormField label="Tanggal Mulai" icon={CalendarDays}><input type="datetime-local" value={form.startDate} onChange={(e) => updateField("startDate", e.target.value)} className="form-input" /></FormField>
              <FormField label="Tanggal Selesai" icon={CalendarDays}><input type="datetime-local" value={form.endDate} onChange={(e) => updateField("endDate", e.target.value)} className="form-input" /></FormField>
              <FormField label="Jarak (KM)" icon={MapPin}><input type="number" min="0" step="0.01" value={form.distanceKm} onChange={(e) => updateField("distanceKm", e.target.value)} placeholder="45" className="form-input" /></FormField>
              <FormField label="Harga Tiket" icon={Ticket}><input type="number" min="0" value={form.ticketPrice} onChange={(e) => updateField("ticketPrice", e.target.value)} className="form-input" /></FormField>
              <FormField label="Kuota Peserta" icon={Ticket}><input type="number" min="0" value={form.maxParticipants} onChange={(e) => updateField("maxParticipants", e.target.value)} placeholder="1000" className="form-input" /></FormField>
              <FormField label="Jumlah Doorprize" icon={Gift}><input type="number" min="0" value={form.doorprizeCount} onChange={(e) => updateField("doorprizeCount", e.target.value)} className="form-input" /></FormField>
              <FormField label="Status" icon={FileText}><select value={form.status} onChange={(e) => updateField("status", e.target.value as EventStatus)} className="form-input"><option value="draft">Draft</option><option value="published">Published</option><option value="closed">Closed</option><option value="finished">Finished</option></select></FormField>
              <FormField label="Cover Image URL / Data" icon={ImageIcon}><input type="text" value={form.coverImage} onChange={(e) => updateField("coverImage", e.target.value)} placeholder="Upload flyer atau paste URL gambar" className="form-input" /></FormField>
            </div>

            <div className="mt-5"><FormField label="Deskripsi Event" icon={FileText}><textarea value={form.description} onChange={(e) => updateField("description", e.target.value)} rows={6} className="form-input min-h-[150px] resize-y" /></FormField></div>
            <div className="mt-5"><FormField label="Isi GPX" icon={FileText}><textarea value={form.gpxContent} onChange={(e) => updateField("gpxContent", e.target.value)} rows={5} placeholder="Upload GPX atau paste isi file GPX di sini." className="form-input min-h-[120px] resize-y font-mono text-xs" /></FormField></div>

            <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:justify-end">
              <Link href="/admin" className="flex h-12 items-center justify-center rounded-xl border border-slate-200 px-6 text-sm font-black text-slate-700 hover:bg-slate-50">Batal</Link>
              <button type="submit" disabled={saving} className="flex h-12 items-center justify-center gap-2 rounded-xl bg-purple-700 px-6 text-sm font-black text-white hover:bg-purple-800 disabled:cursor-not-allowed disabled:opacity-60">
                {saving ? <><Loader2 size={18} className="animate-spin" /> Menyimpan...</> : <><Save size={18} /> Simpan Event</>}
              </button>
            </div>
          </form>
          ...
          <aside className="space-y-6">
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <p className="text-sm font-black uppercase tracking-wide text-purple-700">Preview</p>
              <div className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white">
                <div className="relative h-56 bg-gradient-to-br from-purple-50 to-slate-100">
                  ...
                  <span className="absolute left-4 top-4 rounded-full bg-white px-3 py-1 text-xs font-black text-purple-700">{form.eventType}</span>
                  <span className="absolute right-4 top-4 rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">{formatStatus(form.status)}</span>
                </div>
